	...
	<div class="wrapper">
       ...
       <form method="post" action="<?php echo base_url()?>account/block" id="blockForm" name="blockForm">
    	...
    	<ul class="nav nav-tabs main-views">
		    ...
		    <li class="active"><a href="<?php echo base_url()?>redirect/admin_view3"><i class="icon-time icon-large default"></i> Inactive Accounts</a></li>
					...
					<div class="col-md-12 content">
						...
						if(isset($_POST['searchUsers'])){
							if($_POST['searchUsers']=='')
								$siteUsers = @$controller->getAllExpiredUsers();
							else
								$siteUsers = @$controller->searchExpiredUsers($_POST['searchUsers']);
						}
						else{
							$siteUsers = @$controller->getAllExpiredUsers();
						}
						...
						if($siteUser != null && isset($_POST['searchUsers'])){
							echo '<tr style="font-size:80%;"><td colspan="4">Search Results for "'.$_POST['searchUsers'].'"....<a href="admin-view3.php">CLICK HERE</a> to reload all inactive accounts</td></tr>';
						}
						while($siteUser != null)
						{
							//echo '<form name="form'.$counter.'" action="admin-view.php" method="POST">';
							echo '<tr>';
							echo '<td><a href="#responsive_fileActionAdmin3" data-toggle="modal" onclick="selectIndex('.$counter.')">'.$siteUser->getUserName().'</td>';
						...
						if($counter == 0){
							...
							echo '<td colspan=3><h2 style="color: #000; font-weight:bold;">User not found.</h2><br>
												Developer or Reviewer not found. Please <a href="admin-view3.php" class="btn btn-success">CLICK HERE</a> to refresh the list.</td>';
							echo '</tr>';
						}
