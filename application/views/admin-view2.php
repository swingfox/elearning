...
<html lang="en">
<head>
<title>LOOP | Admin - Reviewer Accounts</title>
...
<body>	
	...
	<div class="wrapper">
		...
		<!-- <form method="post" id="blockForm" name="blockForm" action="blockScript.php">
			<div id="responsive_fileActionAdmin2" class="modal hide fade" tabindex="-1" data-width="760">
			    ...
			    <div class="modal-body">
				    <div class="row-fluid">
					    <div class="span10 offset1">
						    <label class="file-action">What would you like to do with this reviewer account?</label>
					    </div>
				    </div>
			    </div>
        ...
        <form method="post" action="<?php echo base_url()?>account/block" id="blockForm" name="blockForm">
            <div class="modal fade" id="responsive_fileActionAdmin2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    	...
    	<ul class="nav nav-tabs main-views">
		    ...
		    <li class="active"><a class="active-tab" href="<?php echo base_url()?>redirect/admin_view2"><i class="icon-edit icon-large default"></i> Reviewers</a></li>
					...
					<div class="col-md-12 content">
						...
						<div class="table-responsive">
							...
							if($_POST['searchUsers']=='')
								$siteUsers = @$controller->getAllSiteUsers();
							else
								$siteUsers = @$controller->searchSiteUsersRev($_POST['searchUsers']);
						}
						else{
						...
						if($siteUser != null && isset($_POST['searchUsers'])){
							echo '<tr style="font-size:80%;"><td colspan="4">Search Results for "'.$_POST['searchUsers'].'"....<a href="admin-view2.php">CLICK HERE</a> to reload all reviewer accounts</td></tr>';
						}
						while($siteUser != null)
						{
							//echo '<form name="form'.$counter.'" action="admin-view.php" method="POST">';
							echo '<tr>';
							echo '<td><a href="#responsive_fileActionAdmin2" data-toggle="modal" onclick="selectIndex('.$counter.')">'.$siteUser->getUserName().'</td>';
						...
						if($counter == 0){
							echo '<tr>';
							echo '<td>'.'</td>';
							echo '<td colspan=3><h2 style="color: #000; font-weight:bold;">Reviewer not found.</h2><br>
												Reviewer not found. Please <a href="admin-view2.php" class="btn btn-success">CLICK HERE</a> to refresh the list.</td>';
